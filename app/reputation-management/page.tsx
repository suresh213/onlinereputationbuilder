...
function StickyWA() {
  ...
  return (
    <div className="fixed bottom-4 left-0 right-0 z-50 px-4 md:hidden flex justify-center">
      <a
        ...
        className="w-full max-w-sm flex items-center justify-center gap-2.5 py-3.5 px-5 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm shadow-[0_4px_25px_rgba(16,185,129,0.5)] border border-emerald-400/30 transition-all active:scale-95"
        onClick={() => {
      ...
      >
        ...
        <span>Instant Confidential WhatsApp Chat</span>
      </a>
    </div>
  );
}
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border border-zinc-800 rounded-xl bg-zinc-900/50 overflow-hidden transition-all">
      <button
        onClick={() => setOpen(!open)}
        className="w-full py-4 px-5 text-left flex items-center justify-between gap-4 text-white font-semibold text-sm sm:text-base hover:text-yellow-400 transition-colors"
      >
        <span>{q}</span>
      ...
      {open && (
        <div className="px-5 pb-5 text-zinc-400 text-sm leading-relaxed border-t border-zinc-800/60 pt-3">
          {a}
        </div>
      )}
    </div>
...
export default function ReputationManagementPage() {
  return (
    <main className="min-h-screen bg-zinc-950 font-sans selection:bg-yellow-400/30 text-zinc-100">
      ...
      {/* ── HERO SECTION & STICKY CONVERSION FORM ──────────────── */}
      <section className="relative pt-10 pb-16 lg:pt-14 lg:pb-24 overflow-hidden">
        {/* Ambient lighting */}
        <div className="absolute top-0 left-1/4 w-[600px] h-[400px] bg-yellow-500/10 rounded-full blur-[140px] pointer-events-none" />
        <div className="absolute top-1/3 right-10 w-[450px] h-[350px] bg-blue-600/10 rounded-full blur-[130px] pointer-events-none" />

        <div className="max-w-6xl mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-start">

            {/* LEFT: Problem / Solution Authority Copy (7 cols) */}
            <div className="lg:col-span-7 pt-2">
              ...
              <p className="text-zinc-300 text-base sm:text-lg mb-7 leading-relaxed font-normal">
                Negative search results, defamatory articles, and malicious reviews destroy business revenue and personal credibility. We forensically audit, de-index, and permanently suppress damaging digital content with <strong className="text-white">100% legal confidentiality</strong>.
              </p>
              ...
              <ul className="space-y-3.5 mb-8">
                {[
                  ...
                  <li key={point} className="flex items-start gap-3 text-zinc-200 text-sm sm:text-base">
                    <span className="mt-1 flex-shrink-0 w-5 h-5 rounded-full bg-yellow-400/15 border border-yellow-400/40 flex items-center justify-center">
              ...
              {/* Live Proof Metrics */}
              <div className="grid grid-cols-3 gap-3 border-t border-zinc-800/80 pt-6 max-w-lg">
                {[
                  { end: 1200, suffix: "+", label: "Clients Protected" },
                  { end: 98, suffix: "%", label: "Removal Success" },
                  { end: 12, suffix: "+", label: "Years Experience" },
                ].map(({ end, suffix, label }) => (
                  <div key={label} className="text-left">
                    <p className="font-heading text-2xl sm:text-3xl font-black text-yellow-400">
              ...
              <div className="mt-8 hidden sm:flex items-center gap-4">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2.5 px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm transition-all shadow-lg shadow-emerald-600/25 border border-emerald-400/30 hover:scale-[1.02]"
                  onClick={() => {
                    if (typeof window !== "undefined" && typeof (window as any).gtag === "function") {
                      (window as any).gtag("event", "whatsapp_click", {
                        event_category: "Lead",
            ...
            {/* RIGHT: Dedicated High-Conversion Form Card (5 cols) */}
            <div id="audit-form" className="lg:col-span-5 sticky top-20">
              <div className="rounded-2xl border border-yellow-500/30 bg-zinc-900/90 backdrop-blur-md shadow-2xl p-6 lg:p-7 relative overflow-hidden">
                {/* Form header */}
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[10px] font-extrabold uppercase tracking-widest text-yellow-400 bg-yellow-400/10 px-2.5 py-0.5 rounded border border-yellow-400/20">
                      Zero Obligation
                    </span>
                  ...
                  <p className="text-zinc-400 text-xs mt-1">
                    Submit your concern below. A Senior Reputation Director will review your case and share a removal roadmap within 60 minutes.
                  </p>
                </div>

                {/* Form Component with ReCAPTCHA + Conversion tracking */}
                <ContactForm dark />
              </div>
            </div>
      ...
      {/* ── THE 5 PILLARS OF REPUTATION DEFENSE ────────────────── */}
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <h2 className="font-heading text-2xl sm:text-3xl font-extrabold text-white">
              What We Remove &amp; Protect
            </h2>
            <p className="text-zinc-400 text-sm mt-2">
              From corporate crisis suppression to platform-level review removals, we protect every digital touchpoint.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={card.title}
                className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 hover:border-yellow-500/40 transition-all hover:-translate-y-1"
              >
                <div className="text-3xl mb-3">{card.icon}</div>
                <h3 className="font-heading font-bold text-white text-base sm:text-lg mb-2">
                  {card.title}
                </h3>
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <h2 className="font-heading text-2xl sm:text-3xl font-extrabold text-white mb-3">
            How We Clean &amp; Restore Your Reputation
          </h2>
          <p className="text-zinc-400 text-sm mb-12">
            A battle-tested 3-stage process engineered for speed, legality, and permanence.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
            {[
              ...
              <div
                key={step}
                className="relative rounded-2xl border border-zinc-800/80 bg-zinc-900/50 p-6 overflow-hidden"
              >
                <span className="text-5xl font-black text-yellow-400/10 absolute top-2 right-4 select-none font-heading">
                  {step}
                </span>
                <div className="w-8 h-8 rounded-lg bg-yellow-400/15 border border-yellow-400/30 flex items-center justify-center mb-4">
      ...
      <section className="py-16 bg-zinc-900/40 border-y border-zinc-800/80">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-10">
            ...
            <h2 className="font-heading text-2xl sm:text-3xl font-extrabold text-white">
              Trusted by Leaders Across India &amp; Worldwide
            </h2>
          </div>
            ...
            {[
              {
                stars: 5,
                quote:
                  "43 fake 1-star reviews hit our corporate Google profile within 48 hours. Online Reputation Builder audited the coordinated attack, worked with Google support, and removed 39 reviews in 18 days. Rating went from 2.8 back to 4.7.",
              ...
              <div key={t.author} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 flex flex-col justify-between">
                <div>
                ...
                <div className="border-t border-zinc-800 pt-4">
                  ...
                  <p className="text-zinc-500 text-xs mt-0.5">{t.industry}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── FINAL HIGH-URGENCY CONVERSION STRIP ────────────────── */}
      <section className="py-14 border-t border-zinc-800/80 bg-gradient-to-b from-zinc-950 to-zinc-900">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <a
              href="#audit-form"
              className="btn-gold w-full sm:w-auto px-8 py-3.5 text-sm font-bold shadow-xl shadow-amber-500/20"
            >
              Request Confidential Audit →
            </a>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl bg-emerald-600/15 border border-emerald-500/40 text-emerald-400 font-bold text-sm hover:bg-emerald-600/25 transition-all"
              onClick={() => {
                if (typeof window !== "undefined" && typeof (window as any).gtag === "function") {
                  (window as any).gtag("event", "whatsapp_click", {
                    event_category: "Lead",
